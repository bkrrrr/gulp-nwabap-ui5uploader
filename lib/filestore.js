...
FileStore.prototype.syncFile = function (sFile, sModif, sCwd, fnCallback) {
    ...
    me._sendRequest(oRequest, function (oResponse) {
        if (oResponse.error) {
            if (me._oOptions.ui5.transport_use_locked && oResponse.body) {
                let aMatched = oResponse.body.match(/.{3}K\d{6}/g);
                if (aMatched && aMatched.length > 0) {
                    log('NW ABAP UI5 Uploader: Warning: the current BSP Application was already locked in ' + aMatched[0] + '. Now we are using transport ' + aMatched[0] + ' instead of ' + me._oOptions.ui5.transportno + '.');
